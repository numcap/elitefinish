...
const Navbar = () => {
	...
	return (
		<>
			...
			<div className={`fixed top-0 left-0 h-screen w-screen z-50 flex flex-col justify-center items-center gap-5 bg-black transition-all duration-500 ease-in-out transform ${hamburgerModal ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10 pointer-events-none"}`}>
					<Image src='/icons/logo.png' width={150} height={200} alt='Logo' className="mb-10" />
					<Link
					...
					<ImCross className="w-8 h-8 mt-6 cursor-pointer" onClick={() => setHamburgerModal(false)} />
			</div>
		</>
	);
};
